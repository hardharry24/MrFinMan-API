<?php
	...
	{*/
		...
		if ($todo == "LIST")
		{
			$sql = "SELECT * FROM `priorities` JOIN category ON category.categoryId = priorities.categoryId WHERE priorities.isActive = 1 ORDER BY categoryDesc ASC";
		...
		else if ($todo == "INSERT")
		{
			...
			$sql = "SELECT * FROM priorities WHERE categoryId = '".$categoryId."' AND isActive=1";
			$result = mysqli_query($connect,$sql);
			...
			{
				$sql = "INSERT INTO `priorities` (`categoryId`, `percentage`, `isActive`) VALUES ( '".$categoryId."', '".$percentage."', '1')";
				$result = mysqli_query($connect,$sql);
				if ($result)
				{
					$temp['code'] = '1';
