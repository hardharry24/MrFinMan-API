<?php
	...
	{*/
		...
		$response = array();
		$sql = "SELECT category.categoryId,SUM( `amount`) as amount, category.categoryDesc, expense.dateCreated FROM `expense` JOIN category ON expense.categoryId= category.categoryId INNER JOIN user ON user.userId = expense.userId where username ='".$username."' AND DATE_FORMAT(expense.dateCreated,'%d/%m/%Y') = '".$date."' AND expense.isDeleted = 0 GROUP BY category.categoryId";
		$result = mysqli_query($connect,$sql);
